...
class ChatGPTHistoryTool:
    ...
    async def search_conversations(
        ...
        if self._is_cache_valid():
            logger.info("Using cached ChatGPT conversation data")
        ...
        try:
            ...
            # If we still don't have fresh data, use what we have
            if self.conversation_cache:
                logger.warning("Using stale cached ChatGPT conversation data")
                return self._search_cached_conversations(keywords, days, max_results)
            else:
                logger.warning("No ChatGPT conversation data available")
                return []
